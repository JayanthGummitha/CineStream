...
import { getDocumentariesContent } from '@/lib/movie-service';

// Documentary specific categories
const DOCUMENTARY_CATEGORIES = [
  { id: "featured", name: "Featured Documentaries" },
...
export default function DocumentariesPage() {
  ...
  const [featuredDocs, setFeaturedDocs] = useState<Movie[]>([]);
  ...
  useEffect(() => {
    const loadDocumentaries = async () => {
      try {
        ...
        // Fetch documentaries specific content
        const docContent = await getDocumentariesContent();

        setFeaturedDocs(docContent.featured);
        setCategoryContent({
          "Featured Documentaries": docContent.featured,
          "Nature & Wildlife": docContent.nature,
          "History & Biography": docContent.history,
          "Science & Technology": docContent.science,
          "True Crime": docContent.trueCrime
        });
      } catch (error) {
  ...
  const filteredContent = allContent.filter(item => {
    ...
    const matchesCategory = selectedCategory === 'all' || 
      DOCUMENTARY_CATEGORIES.find(c => c.name === selectedCategory)?.id === selectedCategory;
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="text-white/60">Loading documentaries...</p>
          </div>
        </div>
        <Footer />
      </div>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        {/* Hero Section with Featured Documentaries */}
        {featuredDocs.length > 0 && (
          <DocumentaryHeroSection
            featuredContent={featuredDocs}
        ...
        <section className="full-width-minimal py-8 bg-background">
          <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <Input
                type="search"
                placeholder="Search documentaries, topics..."
                value={searchQuery}
            ...
            <div className="flex items-center space-x-3">
              <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                ...
                <SelectContent className="bg-black/90 border-white/20">
                  ...
                  {DOCUMENTARY_CATEGORIES.map((category) => (
          ...
          <div className="mt-6">
            <p className="text-white/70">
              Showing {filteredContent.length} {filteredContent.length === 1 ? 'documentary' : 'documentaries'}
          ...
          {filteredContent.length === 0 && (
            <div className="text-center py-16">
              <div className="max-w-md mx-auto space-y-4">
                ...
                <h3 className="text-2xl font-bold text-white">No documentaries found</h3>
            ...
            <section className="mt-16 py-16 text-center bg-gradient-to-br from-black/60 to-black/80 backdrop-blur-xl border border-white/10 rounded-2xl">
              <div className="max-w-3xl mx-auto space-y-6 px-6">
                ...
                <p className="text-lg text-white/80">
                  Get unlimited access to our award-winning documentary collection.
                  Educational, inspiring, and eye-opening content from around the world.
                </p>
...
interface DocumentaryCategorySectionProps {
  category: { id: string; name: string };
  content: Movie[];
  isAuthenticated: boolean;
}

function DocumentaryCategorySection({ category, content, isAuthenticated }: DocumentaryCategorySectionProps) {
  return (
    <MovieCarousel
      title={category.name}
      movies={content}
...
interface DocumentaryHeroSectionProps {
  featuredContent: Movie[];
  isAuthenticated: boolean;
}

function DocumentaryHeroSection({ featuredContent, isAuthenticated }: DocumentaryHeroSectionProps) {
        ...
        <div className="full-width-minimal">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 w-full">
            ...
            <div className="lg:col-span-8 xl:col-span-7">
              <div className="max-w-3xl h-full flex flex-col justify-center space-y-8">
                ...
                <div>
                  ...
                  {/* Documentary Badges and Metadata */}
                  <div className="flex items-center space-x-4 text-white/90">
                    {/* Badges */}
                    <div className="px-3 py-1 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-full text-white text-xs font-bold uppercase tracking-wider">
                      Documentary
                    </div>
                    {currentMovie.isNew && (
                ...
                <div className="flex items-center space-x-4">
                  <Button
                    size="lg"
                    className="h-14 px-8 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold rounded-2xl shadow-2xl hover:shadow-emerald-500/25 transition-all duration-300"
                  >
                  ...
                  <Button
                    ...
                    asChild
                  >
                    <Link href={createDetailUrl('documentaries', currentMovie.id, currentMovie.title)}>
                     <EllipsisVertical/>
                    </Link>
                  </Button>
                </div>
            ...
            <div className="hidden lg:block lg:col-span-4 xl:col-span-5">
              {/* This space is reserved for the absolutely positioned documentary carousel */}
            </div>
          </div>
        </div>
      </div>

      {/* Bottom Right - Documentary Navigation Carousel */}
                ...
                <div className="relative w-[100px] h-[140px] rounded-2xl overflow-hidden border transition-all duration-300">
                  ...
                  {carouselIndex === 0 && (
                    <div className="absolute top-3 left-3">
                      <div className="px-2 py-1 bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-[8px] font-bold rounded-full uppercase tracking-wider">
                        Next
                      </div>
                    </div>
                  )}
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="w-12 h-12 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-full flex items-center justify-center shadow-2xl">
